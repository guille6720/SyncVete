-- Waiting Room Phase 11: portal ETA minutes_per_patient (clinic override or measured avg).
-- STAGING FIRST. Additive. Does not expose internal_notes or full org settings.

DROP FUNCTION IF EXISTS public.list_owner_portal_waiting_room(DATE);
    ...
    COALESCE(
      ...
      (
        SELECT GREATEST(
          ...
          LEAST(
            120,
            ROUND(AVG(EXTRACT(EPOCH FROM (w3.called_at - w3.checked_in_at)) / 60.0))::INTEGER
          )
        )
        FROM public.waiting_room_entries w3
        WHERE w3.organization_id = w.organization_id
          AND w3.branch_id = w.branch_id
          AND w3.deleted_at IS NULL
          AND w3.called_at IS NOT NULL
          AND w3.checked_in_at >= v_day_start
          AND w3.checked_in_at < v_day_end
          AND w3.called_at > w3.checked_in_at
        HAVING COUNT(*) >= 1
      ),
      15
    ) AS minutes_per_patient
...
COMMENT ON FUNCTION public.list_owner_portal_waiting_room(DATE) IS
  'Portal tutor: waiting-room entries for the authenticated owner only (no internal_notes). Includes minutes_per_patient for ETA.';
