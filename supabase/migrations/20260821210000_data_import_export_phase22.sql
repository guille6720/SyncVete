-- Data Import/Export Phase 22: branches export + import + provenance.
-- Additive. Staging first. Tenant-safe. Never auto-promote is_main.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_branches_org_source_unique
  ON public.branches (organization_id, source_system, source_record_id)
...
CREATE INDEX IF NOT EXISTS idx_branches_import_batch
  ON public.branches (import_batch_id)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'created_rows', jsonb_build_object(
      'branches', (SELECT COUNT(*) FROM public.data_import_created_rows c LEFT JOIN public.branches b ON b.id = c.entity_id AND b.organization_id = c.organization_id WHERE c.organization_id = v_org AND c.entity_type = 'branches' AND b.id IS NULL),
    ...
    'id_map', jsonb_build_object(
      'branches', (SELECT COUNT(*) FROM public.data_import_id_map m LEFT JOIN public.branches b ON b.id = m.internal_id AND b.organization_id = m.organization_id WHERE m.organization_id = v_org AND m.entity_type = 'branches' AND b.id IS NULL),
...
BEGIN
  ...
  IF COALESCE(p_dry_run, true) THEN
    ...
    WHERE c.organization_id = v_org
      AND (
        (c.entity_type = 'branches' AND NOT EXISTS (SELECT 1 FROM public.branches b WHERE b.id = c.entity_id AND b.organization_id = v_org))
        OR (c.entity_type = 'owners' AND NOT EXISTS (SELECT 1 FROM public.owners o WHERE o.id = c.entity_id AND o.organization_id = v_org))
