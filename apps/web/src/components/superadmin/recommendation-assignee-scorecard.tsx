...
import { exportSuperadminRecommendationAssigneeScorecardCsv } from '@/actions/superadmin';
import type { RecommendationAssigneeScorecard } from '@/lib/plan-recommendations';
...
export function SuperadminRecommendationAssigneeScorecard({
  scorecard,
}: {
  scorecard: RecommendationAssigneeScorecard | null;
}) {
  ...
  async function downloadCsv() {
    setMessage(null);
    const result = await run(() => exportSuperadminRecommendationAssigneeScorecardCsv());
    if (!result) return;
    ...
    anchor.href = url;
    anchor.download = `syncvete-assignee-scorecard-${new Date().toISOString().slice(0, 10)}.csv`;
    anchor.click();
  ...
  return (
    <Card id="scorecard-responsables">
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div>
            <CardTitle>Scorecard por responsable</CardTitle>
            <CardDescription>
              Pipeline y cierres por Superadmin asignado. No cambia planes.
          ...
          <tbody>
            {rows.map(({ key, label, href, row }) => (
              <tr key={key} className="border-b last:border-0">
                ...
                <td className="py-2.5 pr-3">{row.withFollowUp}</td>
